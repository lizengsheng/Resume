...
首页会自动读取标记为精选的项目。没有封面时会自动显示文字占位图，没有指标或标签时页面也能正常生成。
...
首页在个人资料区下方使用“项目 / 关于”分段标签，“项目”默认展示。项目面板直接显示精选项目，关于面板只显示技术能力和教育背景。修改项目 Markdown 后，项目标签会自动更新；修改个人资料或关于内容时只需编辑配置文件。禁用 JavaScript 时两个标签的内容会按顺序全部显示。
